...
Profesor,Profesoremail AS "Profesor email",CAT1,CAT2,CAT3,CAT4,CAT5,CAT6,CAT7, MAX(fin)
 ...
 INNER JOIN {customfield_field} cff ON cff.id = cfd.fieldid
  WHERE ctxt.instanceid = c.id) TipoAula,
  ...
  AND cest.id = c.id
  GROUP BY cest.id
  Order BY cest.id asc) Estudiantesactivoshoy,
  ...
  AND cpro.id = c.id
  GROUP BY cpro.id
  Order BY cpro.id asc) Profesoremail,
  ...
  AND (c.shortname COLLATE UTF8_GENERAL_CI LIKE "%-V-I-%" OR c.shortname COLLATE UTF8_GENERAL_CI LIKE "%-M-I-%" OR c.shortname COLLATE UTF8_GENERAL_CI LIKE "%-V-E-%")
  AND rest.shortname = "student"

GROUP BY c.id, DATE_FORMAT(FROM_UNIXTIME(ueest.timeend), '%d/%m/%Y' )
ORDER BY c.id, fin DESC
 
) aulas GROUP BY Id_curso