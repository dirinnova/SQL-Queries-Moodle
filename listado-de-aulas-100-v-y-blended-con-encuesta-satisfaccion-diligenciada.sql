/* Listado de aulas 100% V y Blended con encuesta satisfacción diligenciada

Esta consulta solo devuelve el listado de aulas solo 100% virtual y blended con la fecha fin de los estudiantes y que tienen encuesta de satisfacción diligenciada minimo por un participante
...
*/
  ...
  survey.name nombreencuesta,

  (
    SELECT COUNT(DISTINCT fipreg.id) Preguntas
...
*/
  ...
  INNER JOIN {user_enrolments} ueest on ueest.userid = uest.id and ueest.enrolid = eest.id
  INNER JOIN {feedback} survey on survey.course = c.id
  INNER JOIN {feedback_completed} surveycomp on surveycomp.feedback = survey.id
