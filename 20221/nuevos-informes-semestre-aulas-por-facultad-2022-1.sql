/* Nuevos informes semestre Aulas por Facultad 2022-1 */
SELECT distinct ccc.name as "Id","2022" as "AÑO","I" as "PERIODO/SEMESTRE",REPLACE(ccc.name,"Facultad de ","") as "FACULTAD/DEPENDENCIA",

IF(
    ccc.name = "Facultad de Derecho" /* id = 5 */
    OR ccc.name = "Instituto de Estudios Interdisciplinarios Richard Tovar Cárdenas" /* id = 1511 */
    OR ccc.name = "Departamento de Matemáticas" /* id = 339 */
    OR ccc.name = "Centro de Idiomas y Cultura", /* id = 374 */
    (
        SELECT COUNT(distinct c.id) as "Aulas activas derecho Cal AB"
        FROM mdl_course c
        ...
        (
            (
                (
                    (DATE_FORMAT(FROM_UNIXTIME(ue.timestart), '%Y-%m-%d' ) >= "2022-01-01" AND DATE_FORMAT(FROM_UNIXTIME(ue.timeend), '%Y-%m-%d' ) <= "2023-01-30") /* PREGRADO CAL A */
                    OR 
