...
) AS "Estudiantes 2023-1", /* Aulas con estudiantes activos para el periodo 2023-1*/ 
...
(
    ...
    IF(  /* Condicional */
        ...
        , /* Solo se muestran aulas con estudiantes con fecha finalización 15/06/2023 se valida 2023-06-16-00:59*/ 
            DATE_FORMAT(FROM_UNIXTIME(ueest.timeend), '%Y-%m-%d-%H:%i' ) >= "2023-06-16-01:00" AND REPLACE(SUBSTRING(SUBSTRING_INDEX(ccest.path, "/", 2),LENGTH(SUBSTRING_INDEX(ccest.path, "/", 2-1)) + 1),"/", '') = 2 /* PREGRADO */
    )
    AND rest.shortname = "student"
    AND cest.id = c.id
    GROUP BY cest.id
    ORDER BY cest.id ASC
) AS "Estudiantes pregrado fecha cierre mayor 15jun2023", /* Aulas con estudiantes activos fecha de cierre mayor 15 junio 2023 */ 
...
(
    ...
    GROUP BY cest.id
    ORDER BY cest.id ASC
) AS "Est CAL AB derecho 2023-1", /* Aulas con estudiantes activos Pregrado Calendario A & B 2023-1 */ 
