/* Nuevos informes semestre Aulas por Facultad 2023-1 (2023-01-01)*/
SELECT (@cnt := @cnt + 1) AS Id, t.anio AS "AÑO", t.periodosemestre AS "PERIODO/SEMESTRE",t.facultaddependencia AS "FACULTAD/DEPENDENCIA", t.aulasactivas AS "AULAS ACTIVAS", t.pregrado AS "PREGRADO", t.posgrado AS "POSGRADO", t.educacioncontinuada AS "EDUCACIÓN CONTINUADA",t.fechacortedatos AS "FECHA DE CORTE DE DATOS (dd/mm/aaaa)"
FROM 
(
    SELECT DISTINCT ccc.name AS idfacultades, "2023" AS anio, "I" AS periodosemestre, REPLACE(ccc.name,"Facultad de ","") AS facultaddependencia,
    ...
    (
        ...
        = ccc.name
        ORDER BY c.id ASC
    ) AS aulasactivas, /* Aulas con estudiantes activos para el periodo 2023-1*/ 
    ...
    (
        ...
        = ccc.name
        ORDER BY c.id ASC
    ) AS pregrado, /* Aulas con estudiantes activos para el periodo 2023-1*/ 
    ...
    (
        ...
        INNER JOIN mdl_enrol e ON e.courseid =c.id
        INNER JOIN mdl_user_enrolments ue ON ue.userid = u.id AND ue.enrolid = e.id
        WHERE r.shortname = "student" AND DATE_FORMAT(FROM_UNIXTIME(ue.timestart), '%Y-%m-%d' ) >= @fechainiciosemestral
