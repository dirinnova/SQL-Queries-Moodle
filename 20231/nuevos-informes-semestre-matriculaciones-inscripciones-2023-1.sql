/* Nuevos informes semestre Matriculaciones - Inscripciones 2023-1 */
SELECT (@cnt := @cnt + 1) AS Id, "2023" AS "AÑO","I" AS "PERIODO/SEMESTRE", REPLACE(ccc.name,"Facultad de ","") AS "FACULTAD/DEPENDENCIA", (SELECT cat.name FROM mdl_course_categories cat WHERE cat.id = ccc.parent) Nivel,
