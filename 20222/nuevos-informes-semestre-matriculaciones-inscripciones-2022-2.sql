/* Nuevos informes semestre Matriculaciones - Inscripciones 2022-2 */
SELECT (@cnt := @cnt + 1) AS Id, "2022" AS "AÑO","II" AS "PERIODO/SEMESTRE", REPLACE(ccc.name,"Facultad de ","") AS "FACULTAD/DEPENDENCIA", (SELECT cat.name FROM mdl_course_categories cat WHERE cat.id = ccc.parent) Nivel,
