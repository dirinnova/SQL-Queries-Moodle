/* Listado de aulas de Miscellaneous y Aulas capacitación docente */
...
WHERE 
(
  REPLACE(SUBSTRING(SUBSTRING_INDEX(cc.path, "/", 2),LENGTH(SUBSTRING_INDEX(cc.path, "/", 2-1)) + 1),"/", '') = 1 /* Miscellaneous */
  OR 
  REPLACE(SUBSTRING(SUBSTRING_INDEX(cc.path, "/", 2),LENGTH(SUBSTRING_INDEX(cc.path, "/", 2-1)) + 1),"/", '') = 541 /* Aulas capacitación docente */
)
