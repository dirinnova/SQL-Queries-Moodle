...
WHERE cfo.courseid = c.id and cfo.name="numsections" ORDER BY cfo.id DESC LIMIT 1) "Unidades",
 ...
 c.format Formato, c.fullname Curso, c.shortname NombreCorto, c.visible CursoVisible,
CASE
...
AND cest.id = c.id
GROUP BY cest.id
Order BY cest.id asc) "Estudiantes activos mayor 2021-01-01",
