...
WHERE 
(
DATE_FORMAT(FROM_UNIXTIME(ueest.timestart), '%Y-%m-%d' ) <= "2021-07-16"
AND DATE_FORMAT(FROM_UNIXTIME(ueest.timeend), '%Y-%m-%d' ) >= "2021-07-19"
)

AND (rest.shortname = "student")
AND cest.id = c.id
GROUP BY cest.id
Order BY cest.id asc) "Estudiantes acceso 16 jul al 19 jul",
...
AND cpro.id = c.id
GROUP BY cpro.id
Order BY cpro.id asc), 1, 10000000) "Profesor email",
 
(SELECT GROUP_CONCAT(CONCAT(upro.email) SEPARATOR ' / ') as Coordinador
 FROM {course} cpro
INNER JOIN {context} ctxpro ON ctxpro.instanceid = cpro.id
INNER JOIN {role_assignments} rapro ON ctxpro.id = rapro.contextid
INNER JOIN {role} rpro ON rpro.id = rapro.roleid
...
inner join {enrol} epro on epro.courseid =cpro.id
INNER JOIN {user_enrolments} uepro on uepro.userid = upro.id and uepro.enrolid = epro.id
WHERE (rpro.shortname = "coordinador_inscripcion")
AND cpro.id = c.id
GROUP BY cpro.id
Order BY cpro.id asc) "Coordinador email",
