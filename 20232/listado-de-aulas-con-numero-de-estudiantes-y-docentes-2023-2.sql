/* Listado de aulas con número de estudiantes y docentes 2023-2
Esta consulta devuelve el listado de aulas con total de estudiantes y docentes por cada aula 2023-2 (Derecho Calendario A 2023-2024) y (Pregrado Calendario B 2023-2024) 
...
(
    ...
    IF(  /* Condicional */
        ...
        AND
        (        
            (SELECT cccc.name FROM {course_categories} cccc WHERE cccc.id = (REPLACE(SUBSTRING(SUBSTRING_INDEX(ccest.path, "/", 3),LENGTH(SUBSTRING_INDEX(ccest.path, "/", 3-1)) + 1),"/", ''))) = "Facultad de Derecho" /* Si es de la facultad de derecho */
            OR
            (SELECT cccc.name FROM {course_categories} cccc WHERE cccc.id = (REPLACE(SUBSTRING(SUBSTRING_INDEX(ccest.path, "/", 3),LENGTH(SUBSTRING_INDEX(ccest.path, "/", 3-1)) + 1),"/", ''))) = "Instituto de Estudios Interdisciplinarios Richard Tovar Cárdenas" /* o si es del Instituto de Estudios Interdisciplinarios Richard Tovar Cárdenas */
            OR
            (SELECT cccc.name FROM {course_categories} cccc WHERE cccc.id = (REPLACE(SUBSTRING(SUBSTRING_INDEX(ccest.path, "/", 4),LENGTH(SUBSTRING_INDEX(ccest.path, "/", 4-1)) + 1),"/", ''))) = "Facultad de Derecho" /* o si son Examenes de Clasificación de la Facultad de Derecho */
            OR
            (
                (
                    (SELECT cccc.name FROM {course_categories} cccc WHERE cccc.id = (REPLACE(SUBSTRING(SUBSTRING_INDEX(ccest.path, "/", 3),LENGTH(SUBSTRING_INDEX(ccest.path, "/", 3-1)) + 1),"/", ''))) = "Departamento de Matemáticas" /* o si es del departamento de matematicas */
                    OR
                    (SELECT cccc.name FROM {course_categories} cccc WHERE cccc.id = (REPLACE(SUBSTRING(SUBSTRING_INDEX(ccest.path, "/", 3),LENGTH(SUBSTRING_INDEX(ccest.path, "/", 3-1)) + 1),"/", ''))) = "Centro de Idiomas y Cultura" /* o si es del Centro de Idiomas y Cultura */
                )
                AND
...
) AS "Estudiantes 2023-2", /* Aulas con estudiantes activos para el periodo 2023-1*/ 
...
(
    ...
    IF(  /* Condicional */
        ...
        , /* Solo se muestran aulas con estudiantes con fecha finalización 15/12/2023 se valida 2023-12-16-00:59*/ 
            DATE_FORMAT(FROM_UNIXTIME(ueest.timeend), '%Y-%m-%d-%H:%i' ) >= "2023-12-16-01:00" AND REPLACE(SUBSTRING(SUBSTRING_INDEX(ccest.path, "/", 2),LENGTH(SUBSTRING_INDEX(ccest.path, "/", 2-1)) + 1),"/", '') = 2 /* PREGRADO */
    )
    AND rest.shortname = "student"
    AND cest.id = c.id
    GROUP BY cest.id
    ORDER BY cest.id ASC
) AS "Estudiantes pregrado fecha cierre mayor 15dic2023", /* Aulas con estudiantes activos fecha de cierre mayor 15 diciembre 2023 */ 
...
(
    ...
    GROUP BY cest.id
    ORDER BY cest.id ASC
) AS "Est CAL A derecho 2023-2024", /* Aulas con estudiantes activos Pregrado Calendario A 2023-2024 */ 
...
(
    ...
    IF(  /* Condicional */
        ...
        ( /* aplican los filtros de fechas del calendario B de Derecho */
            (DATE_FORMAT(FROM_UNIXTIME(ueest.timestart), '%Y-%m-%d' ) >= @calBfechainicio AND DATE_FORMAT(FROM_UNIXTIME(ueest.timeend), '%Y-%m-%d' ) <= @calBfechafin) /* Pregrado Calendario B 2023-2024 */
        )
        , 
            ""
    )
    ...
    GROUP BY cest.id
    ORDER BY cest.id ASC
) AS "Est CAL B derecho 2023-2024", /* Aulas con estudiantes activos Pregrado Calendario B 2023-2024 */ 
...
(
    ...
    INNER JOIN {user_enrolments} ueest ON ueest.userid = uest.id AND ueest.enrolid = eest.id
    WHERE 
    IF(  /* Condicional que solo muestran solo los estudiantes de las aulas de derecho pregrado 2023-2 */
        REPLACE(SUBSTRING(SUBSTRING_INDEX(ccest.path, "/", 2),LENGTH(SUBSTRING_INDEX(ccest.path, "/", 2-1)) + 1),"/", '') = 2 /* PREGRADO */
        AND
        (        
            (SELECT cccc.name FROM {course_categories} cccc WHERE cccc.id = (REPLACE(SUBSTRING(SUBSTRING_INDEX(ccest.path, "/", 3),LENGTH(SUBSTRING_INDEX(ccest.path, "/", 3-1)) + 1),"/", ''))) = "Facultad de Derecho" /* Si es de la facultad de derecho */
    ...
    GROUP BY cest.id
    ORDER BY cest.id ASC
) AS "Est CAL AB derecho 2023-2", /* Aulas con estudiantes activos Pregrado Calendario A & B 2023-2 */ 
