/* Esta consulta devuelve el Listado de aulas 100% virtual y Blended creadas ayer
listado-de-aulas-100-virtual-y-blended-creadas-ayer
...
*/
  ...
  SELECT c.id Id_curso, c.fullname Curso, c.shortname NombreCorto, c.format Formato, IF(c.visible=1,"SI","NO") CursoVisible,
  ...
  DATE_FORMAT(CURDATE(), '%d/%m/%Y' ) fechahoy
  FROM {course} c
  ...
  AND DATE_FORMAT(FROM_UNIXTIME(
    ...
    ), '%Y-%m-%d' ) >= DATE_SUB(CURDATE(), INTERVAL 1 MONTH)
