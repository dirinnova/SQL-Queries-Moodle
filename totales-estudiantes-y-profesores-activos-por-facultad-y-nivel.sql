SELECT (@cnt := @cnt + 1) AS Id, ccc.name Facultad, (SELECT cat.name FROM mdl_course_categories cat where cat.id = ccc.parent) Nivel
...
AND DATE_FORMAT(FROM_UNIXTIME(ue.timeend), '%Y-%m-%d' ) >= CURDATE()
